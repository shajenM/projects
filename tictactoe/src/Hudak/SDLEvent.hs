...
sdlMouseButtonToUser (SDL.MouseButtonEventData _ btnMotion _ btnKey _ (P (V2 x y)) ) =
   MouseButton (coordToLoc(gPtToPt(x,y))) (btnKey==SDL.ButtonLeft) (btnMotion==SDL.Pressed)
...
uas = cycle [Nothing, Just (MouseButton (0,0) True True), Nothing]
ts  = [1,2..]:: [Time]
