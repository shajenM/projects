module Hudak.GameEngine where
import Hudak.Util
import Hudak.Fal
import Hudak.Shape
import Hudak.Region
import Hudak.Picture
import Hudak.Reactimate
import Hudak.SDLDraw
import qualified Data.Text as T

-- test reactive Behavior
testReact title beh res = reactimate title beh res picToGraphic
...
-- audio_wetsplash_name = strToTxt  "Wet-splat-sound-effect.mp3"
audio_wetsplash_name = strToTxt  "doubleTing.mp3"
resAudioWetSplash = Audio audio_wetsplash_name
-- button
drawButton ((x1,y1), (x2,y2)) = translate p1 (rectFill  p2 p3) where
    p1 = (constB x1, constB y1)
    p2 = constB(x2-x1)
    p3 = constB(y2-y1)

isInButton p0 bLoc buttonSize = let (p1,p2) = (toRect bLoc buttonSize) in isInRect p0 p1 p2 

buttonPic loc bSize bColor = paintedPicture bColor bPic where
    bRect  = toRect loc bSize
    bPic   = drawButton bRect
-- resources
